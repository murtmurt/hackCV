...
def run():
	...
	while True:
		...
		for (objectID, centroid) in objects.items():
			...
			for rect in rects:
				cv2.rectangle(frame, rect[:2], rect[2:4], (255, 0, 0), 3)
		...
		if (time.time() >= timestamp) & (len(x) != 0):
			timePoints.append(delta * count)
			population.append(x[0])
			dydx = list(np.diff(population) / np.diff(timePoints))
			dictionary = {
				"timestamp": round(time.time()),
				"popChange": x[0] - tempX,
